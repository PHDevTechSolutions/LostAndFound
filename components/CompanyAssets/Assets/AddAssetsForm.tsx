...
const AddContainerForm: React.FC<AddContainerProps> = ({ onCancel, refreshPosts, userName, editData, Location: propLocation }) => {
  ...
  const [PurchasedAmount, setPurchasedAmount] = useState(editData?.PurchasedAmount || "");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    const url = editData ? `/api/CompanyAssets/Assets/EditRecord` : `/api/CompanyAssets/Assets/CreateRecord`;
    ...
    <>
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-4 text-xs">
        ...
        <FormFields
          ...
          DatePurchased={DatePurchased} setDatePurchased={setDatePurchased}
          SupplierBrand={SupplierBrand} setSupplierBrand={setSupplierBrand}
