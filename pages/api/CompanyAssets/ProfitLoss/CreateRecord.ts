...
async function addPurchasing({ 
  ...
  const containerCollection = db.collection("profitloss");

  // Create container data
  const newData = { ReferenceNumber, Location, DateRecord, ModeType, Amount, createdAt: new Date() };
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    const { ReferenceNumber, Location, DateRecord, ModeType, Amount,
     } = req.body;
    ...
    try {
      const result = await addPurchasing({ ReferenceNumber, Location, DateRecord, ModeType, Amount,
      });
